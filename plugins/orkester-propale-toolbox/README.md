...
- **Cohérence du socle** — les décisions structurantes prises en aval (périmètre tranché dans la couverture fonctionnelle, angle arbitré dans la trame) sont répercutées dans le socle et tracées dans `## Décisions retenues` de `progression.md`. Aucun outil consommant le socle ne se lance sur une décision non propagée.
- **Les arbitrages de structure se négocient en amont** — le schéma de regroupement de la couverture fonctionnelle, comme le fil rouge de la trame, se décide avec l'utilisateur dans le fil principal avant de lancer l'outil, jamais en corrigeant le livrable après coup.
