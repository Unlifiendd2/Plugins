...
description: >
  ...
  actionnable et l'écrit dans le fichier final output/revue-[nom-projet].md. Retourne
  uniquement un résumé (scores + verdict + chemin du fichier) à l'orchestrateur, qui ne lira
...
1. **Lire les 3 fichiers d'analyse** aux chemins fournis dans le prompt :
   - Chemin de l'analyse storytelling (`output/tmp/_revue-storytelling-[nom-projet].md`)
   - Chemin de l'analyse cohérence (`output/tmp/_revue-coherence-[nom-projet].md`)
   - Chemin de l'analyse pertinence (`output/tmp/_revue-pertinence-[nom-projet].md`)
