...
## Scénarios hors catalogue

Le catalogue `references/catalogue-sections.md` est la **seule source** de sections : cette version du plugin ne consulte aucune base de connaissances externe. Si le cas présente une **ambiguïté** que les 4 axes ne lèvent pas (produit hybride, périmètre flou entre build et reprise) ou un **scénario non couvert** par le catalogue (conseil pur, audit, cadrage seul, formation, secteur inhabituel), s'appuyer sur les sections du catalogue les plus proches, adapter leur description au cas, et **signaler explicitement l'hypothèse dans le résumé final** (section adaptée hors couverture directe du catalogue). Ne rien inventer qui ne s'appuie pas sur le catalogue ou le fichier contexte.
