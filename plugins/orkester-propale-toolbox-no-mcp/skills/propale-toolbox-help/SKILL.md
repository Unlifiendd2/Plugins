...
description: >-
  Documentation de référence du plugin `orkester-propale-toolbox-no-mcp`. À utiliser pour répondre aux
...
# Aide — plugin `orkester-propale-toolbox-no-mcp`
...
└── agents/
    ...
    ├── skill-executor.md    # Exécute un skill en contexte frais
    └── trame-reviewer.md    # Revue 3 lentilles en contexte frais
```
...
- **Version sans MCP** : cette variante du plugin n'accède à **aucune** base de connaissances externe (pas d'`Orkester-kb`). Le travail s'appuie uniquement sur les fichiers sources fournis, le fichier contexte et le catalogue de sections intégré.
...
- **Aucun accès à une base de connaissances** : cette version sans MCP ne consulte pas `Orkester-kb`. Ni le fil principal ni les sous-agents ne font de recherche live dans les propales gagnées ; tout le travail (fil rouge, trame, revue) s'appuie sur les fichiers sources fournis, le contexte consolidé et le catalogue de sections intégré. Pour un ancrage sur les propales gagnées, utiliser le plugin `orkester-propale-toolbox` (version avec MCP).
